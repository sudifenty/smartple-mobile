-- =====================================================================
-- SmartPLE Mobile — ONE-PROJECT MIGRATION
-- Installs the mobile app (student + admin) into the EXISTING old
-- project (ftykmafgfyqvniacguyi) that the web paywall already uses.
-- 100% ADDITIVE: old web-app tables, functions and triggers are never
-- dropped or renamed. Run ONCE, top to bottom, in the SQL Editor of
-- the OLD project.
-- =====================================================================

-- ---------- 1. EXTEND the existing smartple_profiles (adds only what's missing)
...
-- ---------- 2. CREATE the missing smartple_* tables ----------
...
-- ---------- 4. SIGNUP TRIGGER (uniquely named — the old app's trigger,
--               if any, is left completely alone) ----------
...
-- ---------- 5. ROW LEVEL SECURITY — allow-all "for now" (your spec) ----------
...
-- ---------- 6. FUNCTIONS (none of these exist yet in this project) ----------
...
SELECT u.id, u.id,
       ...
       COALESCE(u.raw_user_meta_data->>'class_level',
...
-- older phone-only rows: make sure nothing shows up nameless in the admin panel
UPDATE public.smartple_profiles
SET display_name = COALESCE(display_name, phone, left(id::text, 8))
WHERE display_name IS NULL;

-- ---------- 8. DEMO QUESTIONS (only if the bank is empty) ----------
...
-- ---------- VERIFY ----------
-- SELECT display_name, class, role FROM public.smartple_profiles ORDER BY role, display_name;
-- SELECT tier, COUNT(*) FROM public.smartple_questions GROUP BY tier ORDER BY tier;
